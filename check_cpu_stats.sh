#!/bin/bash
# ==============================================================================
# CPU Utilization Statistics plugin for Nagios and Icinga2
#
...
#          ./check_cpu_stats.sh -w 70,40,30,10 -c 90,60,40,20
#          ./check_cpu_stats.sh -w 70,40,30,10 -c 90,60,40,20 -i 3 -n 5
# 
# ----------------------------------------------------------------------------------------
#
...
# 2.3.7       | 19.01.22  | amaxi@npsh.hu      | Add cpu_steal alert (suggested: 10%,20%, default: 30%,40%), Tested only on SUSE Linux Enterprise Server 12SP5,15SP2,15SP3, but it should also work with any other distribution.
# Why is %cpu_steal important? https://scoutapm.com/blog/understanding-cpu-steal-time-when-should-you-be-worried
# -----------------------------------------------------------------------------------------
...
LIST_WARNING_THRESHOLD=${LIST_WARNING_THRESHOLD:="70,40,30,30"}
LIST_CRITICAL_THRESHOLD=${LIST_CRITICAL_THRESHOLD:="90,60,40,40"}
INTERVAL_SEC=${INTERVAL_SEC:="1"}
NUM_REPORT=${NUM_REPORT:="3"}
# Plugin variable description
PROGNAME=$(basename $0)
RELEASE="Revision 2.3.7"
...
print_usage() {
  ...
  echo ""
  echo "  -w  Warning threshold in % for warn_user,warn_system,warn_iowait,cpu_steal CPU (default : 70,40,30,10)"
  echo "  Exit with WARNING status if cpu exceeds warn_n"
  echo "  -c  Critical threshold in % for crit_user,crit_system,crit_iowait,cpu_steal CPU (default : 90,60,40,20)"
...
print_help() {
  print_usage
    echo ""
    echo "This plugin will check cpu utilization (user,system,CPU_Iowait,idle,cpu_steal in %)"
    echo ""
  exit 0
}
...
CPUSTEAL_CRITICAL_THRESHOLD=`echo ${TAB_CRITICAL_THRESHOLD[3]}`
fi

# Check that critical value should be greater than warning value
if [ ${TAB_WARNING_THRESHOLD[0]} -ge ${TAB_CRITICAL_THRESHOLD[0]} -o ${TAB_WARNING_THRESHOLD[1]} -ge ${TAB_CRITICAL_THRESHOLD[1]} -o ${TAB_WARNING_THRESHOLD[2]} -ge ${TAB_CRITICAL_THRESHOLD[2]} -o ${TAB_WARNING_THRESHOLD[3]} -ge ${TAB_CRITICAL_THRESHOLD[3]} ]; then
...
if [ ${CPU_USER_MAJOR} -ge $USER_CRITICAL_THRESHOLD ]; then
    ...
    echo "CPU STATISTICS CRITICAL : ${NAGIOS_DATA}"
    exit $STATE_CRITICAL
elif [ ${CPU_STEAL_MAJOR} -ge $CPUSTEAL_CRITICAL_THRESHOLD ]; then
    echo "CPU STATISTICS CRITICAL : ${NAGIOS_DATA}"
    exit $STATE_CRITICAL
# Warning
    ...
    echo "CPU STATISTICS WARNING : ${NAGIOS_DATA}"
    exit $STATE_WARNING
elif  [ ${CPU_STEAL_MAJOR} -ge $CPUSTEAL_WARNING_THRESHOLD ] && [ ${CPU_STEAL_MAJOR} -lt $CPUSTEAL_CRITICAL_THRESHOLD ]; then
    echo "CPU STATISTICS WARNING : ${NAGIOS_DATA}"
    exit $STATE_WARNING
# OK
else
